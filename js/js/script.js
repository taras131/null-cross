...
window.addEventListener(`DOMContentLoaded`, function() {
	...
	function addZero(index){
	    ...
	    playingfield_items[index].classList.add(`busy`,`null`);
	}
	...
	function computedStep() {
		...
		if (countstep === 1){
	        if(!arrnull[4]){
				...
				if (+localStorage.getItem(`human_score`) > +localStorage.getItem(`computed_score`)){
					addCross(8);
				} else {
					addCross(7);
				}	           
		...
		if (countstep > 1 && countstep < 5){
			let x = searchSuccess(arrnull,arrcross);
			if(!(x === -1)){
				addCross(x);
			} else {
				addCross(checkDanger(arrnull,arrcross));
			}
		}
